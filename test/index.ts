...
describe("EnvironmentalAsset", function () {
  ...
  it("should mint assets and update balances", async function () {
    ...
    await environmentalAsset
      .connect(owner)
      .mintAsset(recipient.address, amountToMint);

    const recipientBalance = await environmentalAsset.balances(
      recipient.address
    );
  ...
  it("should transfer assets between addresses", async function () {
    ...
    await environmentalAsset
      .connect(recipient)
      .transferAsset(owner.address, amountToTransfer);

    const recipientBalance = await environmentalAsset.balances(
      recipient.address
    );
    const ownerBalance = await environmentalAsset.balances(owner.address);

    expect(recipientBalance).to.equal(amountToMint - amountToTransfer);
    expect(ownerBalance).to.equal(amountToTransfer);
  });
  ...
  it("should revoke assets after the expiration date", async function () {
    ...
    try {
      await environmentalAsset.connect(owner).revokeAsset();
      const ownerBalance = await environmentalAsset.balances(owner.address);
      const availableSupply = await environmentalAsset.availableSupply;
